...
export function evaluateCSTransition(project: ImplementationProject): { status: TransitionStatus; label: string; reasons: string[] } {
  ...
  if (!isFirstValueValidated(project)) reasons.push('ciclo operacional ainda não validado')
  ...
  return { status: 'ready', label: 'Transição completa', reasons: ['ciclo operacional validado e sem pendências abertas'] }
}
